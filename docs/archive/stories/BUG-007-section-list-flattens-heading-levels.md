...
## Acceptance Criteria
- [x] Define and document heading selection strategy for direct-markdown sources:
  - prefer H1 sections
  - fallback to H2 when no H1 exists
- [x] Section list does not mix all heading levels by default.
- [x] Behavior is test-covered for files with:
  - H1 + H2 + H3 mixed
  - only H2 headings
  - no headings
- [x] Existing MoonReader/chapter behavior remains unchanged.
